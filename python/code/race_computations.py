...
def schedule_data_ingestion(token, intervals={}):
    ...
    schedule_job(
        post_lap_invocations,
        intervals["lap_finish"]["interval"],
        funcs=lap_based_functions,
    )
...
def split_time_info():
    ...
    return {
        (sector["SectorNum"] + 1): round(sector["SectorStartPct"] * 100, 2)
...
def last_lap_data():
    ...
    incidents_incurred = (
        ir["PlayerCarMyIncidentCount"] - state.computation_helpers["last_lap_incidents"]
    )
    state.computation_helpers["last_fuel_level"] = ir["FuelLevel"]
    ...
    schedule.every(5).seconds.do(
        send_lap_data_with_time, pending_data=pending_data
    ).tag("lap_data_delayed")


def send_lap_data_with_time(pending_data):
    """Send lap data with the now-updated lap time"""
    lap_data = {**pending_data, "lap_time": format_lap_time(ir["LapLastLapTime"])}
        ...
        if command in command_map:
            ...
            try:
                ...
                if fuel.endswith("l"):
                    ...
                    fuel_amount = fuel[:-2]
                    fuel_amount = ceil(
                        float(fuel_amount) / 0.745
                    )  # Convert kg to liters (approximation)
                else:
