...
DEFAULT_SETTINGS = {
    ...
    "relative": {"enabled": True, "mode": "interval", "interval": 5},
    "weather": {"enabled": True, "mode": "lap", "interval": 30},
    "incidents": {"enabled": True, "mode": "lap", "interval": 15},
    "tyres": {"enabled": True, "mode": "lap", "interval": 20},
    "pit": {"enabled": True, "mode": "interval", "interval": 10},
}
...
class DataGroupWidget(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        self.enable_check = ttk.Checkbutton(
            left_frame, text="Enabled", variable=self.enabled_var
        )
        ...
        description = DATA_GROUP_DESCRIPTIONS.get(
            self.group_name, "No description available"
        )
        ...
        if self.group_name == "lap_finish":
            ...
            self.lap_radio = ttk.Radiobutton(
                mode_frame, text="End of lap", variable=self.mode_var, value="lap"
            )
...
def get_data_settings_tab(notebook):
    ...
    data_groups["lap_finish"].enabled_var.trace_add(
        "write", update_lap_mode_availability
    )
