...
class StreamingThreadController:
    ...
    def stop_all_threads():
        ...
        if StreamingThreadController.job_thread is not None and StreamingThreadController.job_thread.is_alive():
        ...
        if StreamingThreadController.ir_heartbeat_thread is not None and StreamingThreadController.ir_heartbeat_thread.is_alive():
    ...
    def are_threads_running():
        ...
        return (StreamingThreadController.job_thread is not None and StreamingThreadController.job_thread.is_alive())
...
def _check_heartbeat(test_file=None):
    while not StreamingThreadController.stop_threads and _check_iracing(test_file=test_file):
        sleep(1)
...
def create_lobby_and_stream(lobby_name: str, passcode: str, pit_stop_settings: dict, test_file=None):
    logger.info(f"Creating lobby: {lobby_name}")
    response = _create_lobby(lobby_name=lobby_name, passcode=passcode, pit_stop_settings=pit_stop_settings, callback=execute_commands)
...
def join_lobby_and_stream(lobby_name: str, passcode: str, pit_stop_settings: dict, test_file=None):
    logger.info(f"Joining lobby: {lobby_name}")
    response = _join_lobby(lobby_name=lobby_name, passcode=passcode, pit_stop_settings=pit_stop_settings, callback=execute_commands)
    token = response.get('token', None)
