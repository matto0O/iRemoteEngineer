...
def get_streaming_tab(notebook, debug=False):
    ...
    # ===== Lobby Configuration =====
    config_frame = ttk.LabelFrame(main_container, text="Lobby Configuration", padding="10")
    ...
    def start_streaming_thread(lobby_name, passcode, mode):
        ...
        try:
            ...
            if mode == "create":
                create_lobby_and_stream(lobby_name, passcode, pit_stop_settings, test_file=test_file)
                message = f"Lobby created successfully!\n\nLobby Name: {lobby_name}\nPasscode: {passcode}\n\nShare the passcode with others to join."
            else:
                join_lobby_and_stream(lobby_name, passcode, pit_stop_settings, test_file=test_file)
            ...
            frame.after(0, lambda: messagebox.showerror("Error", f"Failed to start streaming:\n{str(e)}"))
    ...
    def stop_streaming_thread():
        ...
        try:
            ...
            frame.after(0, lambda: messagebox.showinfo("Stopping", "Stopping streaming, please wait..."))
            ...
            frame.after(0, lambda: messagebox.showinfo("Streaming Stopped", "Streaming stopped"))
            frame.after(0, lambda: toggle_buttons(start_enabled=True))
            
        except Exception as e:
            frame.after(0, lambda: messagebox.showerror("Error", f"Error stopping stream:\n{str(e)}"))
