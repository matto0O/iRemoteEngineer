...
class KinesisConsumer:
    ...
    def get_records_by_partition_key(self, partition_key):
        ...
        """
        logging.info(f"Fetching records from stream '{self.stream_name}' with partition key '{partition_key}'...")
...
def main():
    parser = argparse.ArgumentParser(description='Download records from Kinesis by partition key')
    parser.add_argument('--output', '-o', type=str, help='Output JSON file path',
                        default='kinesis_records.json')
    parser.add_argument('--stream', '-s', type=str, help='Kinesis stream name',
